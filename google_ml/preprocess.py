...
def text_array_to_csv(text_array, csv_file, l_char_to_index):
    ...
    with open(csv_file, 'w') as file_csv:
        ...
        for sentence_array in text_array:
            x = np.zeros(WINDOW_SIZE + 1, dtype=int)
            for t, char in enumerate(sentence_array):
                x[t] = int(l_char_to_index[char])
            writer.writerow(x)


def text_to_csv(text, csv_file_train, csv_file_test, l_char_to_index,
                percent_test):
    """
    ...
    # outputs: List of chars, being the char ith
    # the next char to sentence inputs[ith]
...
def csv_to_text(csv_file, index_to_char):
    ...
    with open(csv_file, 'r') as file_csv:
        ...
        for row in reader:
            sentence_csv = ""
            for index in row:
                sentence_csv = sentence_csv + index_to_char[int(index)]
...
class TestPreproccess(unittest.TestCase):
    def test_load(self):
        ...
        text_clean = load_text_clean('../data/Beiras.txt',
                                     chars_to_indices_new)
        text_to_csv(text_clean, "train.unittest", "test.unittest",
                    chars_to_indices_new, PERCENT_TRAIN)
...
if __name__ == "__main__":
    ...
    # Generate files
    text_to_csv(text_clean, FILE_OUTPUT_TRAIN, FILE_OUTPUT_TEST,
                chars_to_indices_new, PERCENT_TRAIN)
